export type MelhorEnvioConfig = {
  ...
  productionPreviewVolume?: { weight: number; height: number; width: number; length: number }
  serviceIds: number[]
}
...
export function readMelhorEnvioConfig(env: NodeJS.ProcessEnv): MelhorEnvioConfig | undefined {
  ...
  const productionPreview = env.MELHOR_ENVIO_PRODUCTION_PREVIEW_FALLBACK_ENABLED === "true"
  ...
  if (env.MELHOR_ENVIO_PRODUCTION_PREVIEW_FALLBACK_ENABLED && !["true", "false"].includes(env.MELHOR_ENVIO_PRODUCTION_PREVIEW_FALLBACK_ENABLED)) {
    throw new Error("MELHOR_ENVIO_PRODUCTION_PREVIEW_FALLBACK_ENABLED must be true or false")
  }
  if (productionPreview && mode !== "production") throw new Error("Production preview dimensions are permitted only in Melhor Envio production")
  if (mode !== undefined && mode !== "sandbox" && mode !== "production") throw new Error("MELHOR_ENVIO_ENV must be sandbox or production")
  ...
  if (!mode || !env.MELHOR_ENVIO_ACCESS_TOKEN || !env.MELHOR_ENVIO_USER_AGENT) {
    throw new Error("Melhor Envio requires MELHOR_ENVIO_ENV, MELHOR_ENVIO_ACCESS_TOKEN and MELHOR_ENVIO_USER_AGENT")
  }
  ...
  if (!serviceIds.length) throw new Error("MELHOR_ENVIO_SERVICE_IDS requires at least one service ID")
  ...
  return {
    ...
    demoVolume: fallback ? demoVolume : undefined,
    productionPreviewVolume: productionPreview ? demoVolume : undefined,
    serviceIds,
  }
}
